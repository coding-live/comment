...
import org.imooc.service.BaseService;
...
public class AdServiceImpl extends BaseService implements AdService {
	...
	public boolean add(AdDto adDto) {
		logger.info("新增广告参数[{}]",ReflectionToStringBuilder.toString(adDto));
		Ad ad = new Ad();
		...
		if (adDto.getImgFile() != null && adDto.getImgFile().getSize() > 0) {
			...
			try {
				...
				logger.info("执行添加广告信息异常 [{}]", e.toString());
				return false;
			}
		} else {
			logger.info("执行添加广告上传文件为空");
			return false;
		}
	}
